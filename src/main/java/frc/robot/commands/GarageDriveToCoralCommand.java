...
public class GarageDriveToCoralCommand extends Command {
  ...
  public GarageDriveToCoralCommand(DriveSubsystem drive, Vision vision) {
    addRequirements(drive);
    this.vision = vision;
    this.drive = drive;
    this.targetPoseSupplier = () -> vision.getCoralPoseTracker().getCoralPose();
    distanceController.setTolerance(Units.inchesToMeters(0.5));
    angleController.setTolerance(Units.degreesToRadians(1.5));
  }

  public GarageDriveToCoralCommand withJoystickRumble(Command rumbleCommand) {
    ...
    Pose2d robot =
        robotPoseOriginal.plus(
            new Transform2d(
                ...
                Rotation2d.kZero));
    if (targetPoseSupplier.get().isPresent()){
      target = targetPoseSupplier.get().get();
    } else {
      return;
    }
  ...
  public boolean isFinished(){
    return targetPoseSupplier.get().isEmpty();
  }
