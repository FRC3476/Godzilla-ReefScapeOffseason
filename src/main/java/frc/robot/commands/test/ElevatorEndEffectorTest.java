...
public class ElevatorEndEffectorTest extends SequentialCommandGroup {
  ...
  public ElevatorEndEffectorTest(Elevator elevator, EndEffector endEffector) {
    addCommands(
        ...
        new WaitUntilCommand(() -> endEffector.isInTolerance(EndEffectorConstants.IDLE_ANGLE_RAD, POSITION_TOLERANCE_RAD))
  ...
  private Command createSafeEndEffectorMovement(EndEffector endEffector, double targetAngleRad) {
    return Commands.either(
        Commands.sequence(
            ...
            new WaitUntilCommand(() -> endEffector.isInTolerance(targetAngleRad, POSITION_TOLERANCE_RAD))
                .withTimeout(MOVEMENT_TIMEOUT_SECONDS)
        ),
        Commands.print("Skipping unsafe EndEffector movement because " + Math.toDegrees(targetAngleRad) + " degrees"),
