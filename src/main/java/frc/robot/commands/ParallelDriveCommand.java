...
public class ParallelDriveCommand extends Command {
  ...
  private final DoubleSupplier parallelSpeedSupplier;
  ...
  public ParallelDriveCommand(
      DriveSubsystem drive,
      Supplier<Pose2d> targetPoseSupplier,
      DoubleSupplier parallelSpeedSupplier) {
    addRequirements(drive);

    this.drive = drive;
    this.targetPoseSupplier = targetPoseSupplier;
    this.parallelSpeedSupplier = parallelSpeedSupplier;
    perpendicularController.setTolerance(Units.inchesToMeters(0.5));
    angleController.setTolerance(Units.degreesToRadians(0.5));
  }

  public ParallelDriveCommand withJoystickRumble(Command rumbleCommand) {
  ...
  public void execute() {
    ...
    double xJoy = parallelSpeedSupplier.getAsDouble();
    xJoy = Util.handleDeadband(xJoy, 0.05);
    xJoy = Math.copySign(xJoy * xJoy, xJoy);

    Translation2d velocity =
        new Translation2d(perpendicularSpeed, xJoy * Constants.DriveConstants.kDriveMaxSpeed);
    velocity = velocity.rotateBy(target.getRotation());
    ...
    Logger.recordOutput("Commands/" + getName() + "/Speeds/Perpendicular", perpendicularSpeed);
    Logger.recordOutput(
        "Commands/" + getName() + "/Speeds/Parallel", parallelSpeedSupplier.getAsDouble());
  ...
  public Trigger atSetpoint() {
    return new Trigger(() -> perpendicularController.atSetpoint() && angleController.atSetpoint());
  }
}
