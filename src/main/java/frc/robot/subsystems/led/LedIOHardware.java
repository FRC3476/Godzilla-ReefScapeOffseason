...
public class LedIOHardware implements LedIO {
    ...
    public LedIOHardware() {
        if (Robot.isReal()) {
            candle =
                    new CANdle(
                            Constants.LEDConstants.ID
                            );
            LEDConfigs ledConfigs = new LEDConfigs().withBrightnessScalar(1.0).withStripType(StripTypeValue.RGB);
            CANdleConfiguration candleConfiguration = new CANdleConfiguration().withLED(ledConfigs);
            PhoenixUtil.tryUntilOk(
        5, () -> candle.getConfigurator().apply(candleConfiguration));
        } else {
            candle = null;
        }
    }
    ...
    public void writePixels(LedState[] pixels) {
        ...
        if (candle != null)
            candle.setControl(new SolidColor(runStart,pixels.length - runStart).withColor(run.getRGBW()));
    }
}
