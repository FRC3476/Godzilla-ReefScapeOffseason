package frc.robot.util.pathplanner.trajectory;
...
import frc.robot.util.pathplanner.events.*;
import frc.robot.util.pathplanner.path.EventMarker;
import frc.robot.util.pathplanner.path.PathPlannerPath;
import frc.robot.util.pathplanner.path.PathPoint;
import frc.robot.util.pathplanner.path.PointTowardsZone;
import frc.robot.util.pathplanner.util.DriveFeedforwards;
import java.util.*;
...
public class PathPlannerTrajectory {
  ...
  public PathPlannerTrajectory(
    ...
    if (path.isChoreoPath()) {
      ...
      // speed.
      double speed =
          startVel.getX() * startHeading.getCos() + startVel.getY() * startHeading.getSin();
      if (speed < 0.0) {
        speed = 0.0;
      }
      startVel = new Translation2d(speed, startHeading);
      startingSpeeds =
          new ChassisSpeeds(startVel.getX(), startVel.getY(), startingSpeeds.omegaRadiansPerSecond);
      ...
      Logger.recordOutput(
          "PathPlanner/EndSpeedTransHeading", states.get(states.size() - 1).heading);
      ...
      for (int i = 1; i < states.size(); i++) {
        ...
        if (Math.abs(sumV) < 1e-6 || Math.abs(state.deltaPos) < 1e-6) {
          ...
          Translation2d centripetalAccelFieldFrame =
              new Translation2d(
                  centripetalAccelMagnitude, state.heading.rotateBy(Rotation2d.kCCW_90deg));
          Translation2d centripetalAccelRobotFrame =
              centripetalAccelFieldFrame.rotateBy(state.pose.getRotation().unaryMinus());
          Translation2d centripetalForceRobotFrame =
              centripetalAccelRobotFrame.times(config.massKG);
          ChassisSpeeds chassisForcesCentripetal =
              new ChassisSpeeds(
          ...
          Translation2d totalChassisAccel =
              new Translation2d(
                  chassisAccelX + centripetalAccelRobotFrame.getX(),
                  chassisAccelY + centripetalAccelRobotFrame.getY());

          Translation2d icrRobotFrame =
              Math.abs(state.curvature) > 1e-6
                  ? new Translation2d(
                      1.0 / Math.abs(state.curvature), centripetalForceRobotFrame.getAngle())
                  : Translation2d.kZero;
          ...
          for (int m = 0; m < config.numModules; m++) {
            ...
            forceXFF[m] =
                wheelForces[m].getX() + centripetalRatio * wheelForcesCentripetal[m].getX();
            forceYFF[m] =
                wheelForces[m].getY() + centripetalRatio * wheelForcesCentripetal[m].getY();
  ...
  private static void reverseAccelPass(
    ...
    for (int i = states.size() - 2; i > 0; i--) {
      ...
      // Rotate so that +x is aligned with heading and +y is aligned with centripetal
      // acceleration.
      Translation2d accelLimitsVec =
          new Translation2d(state.constraints.maxXAccelMPSSq(), state.constraints.maxYAccelMPSSq())
              .rotateBy(state.pose.getRotation())
              .rotateBy(state.heading.unaryMinus());
      double maxAccel =
          Math.min(state.constraints.maxAccelerationMPSSq(), Math.abs(accelLimitsVec.getX()));
      double accel = accelVec.getNorm();
      if (accel > maxAccel) {
        accelVec = accelVec.times(maxAccel / accel);
      }
