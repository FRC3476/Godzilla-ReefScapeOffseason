package frc.robot.util.pathplanner.pathfinding;
  ...
  private PathPlannerPath checkCache(GridPosition start, GridPosition goal) {
    ...
    for (int i = -kDistanceGridCells; i <= kDistanceGridCells; ++i) {
      for (int j = -kDistanceGridCells; j <= kDistanceGridCells; ++j) {
        if (i == 0 && j == 0) continue;
        key =
            new Pair<GridPosition, GridPosition>(new GridPosition(start.x + i, start.y + j), goal);
        ...
        if (goalPath == null) {
          ...
          path =
              PathPlannerPath.concatenatePaths(
                  ...
                  new GoalEndState(
                      goalPath.getIdealStartingState().velocity(), realGoalPose.getRotation()),
